...
vector<vector<string>> findSCC(adj &graph)
{
    ...
    for (auto &pair : collections)
        scc.push_back(pair.second);

    return scc;
}

/*
 * this function checks whether we have a path between two nodes or not
 ...
 * Time Complexity: O(k⋅(V+E)logV), Space Complexity: O(k⋅V+E)
 */
                           ...
                           const string &src, const string &dst, int k)
{
                        ...
                        set<string> &bannedNodes) -> Path
    {
        ...
        while (!pq.empty())
        {
            ...
            for (auto &e : graph[u])
            {
                if (bannedNodes.count(e.to))
                    continue;
                if (bannedEdges.count({u, e.to}))
                    continue;
 ...
 * Time Complexity: O(E⋅(V+E)), Space Complexity: O(V+E)
 */
 ...
 *
 * Time Complexity: O(V+E+VlogV), Space Complexity: O(V+E)
 */
...
/**
 * Returns the minimum set of airport codes that must be connected
 * from the given capital airport to make all airports reachable.
 *
 * The graph is compressed into strongly connected components (SCCs),
 * and a BFS is performed on the SCC graph to identify unreachable
 * components with zero indegree.
 * Time Complexity: O(V+E), Space Complexity: O(V+E)
 */
vector<string> findMissingReachableComponents(adj &graph,
                                              const string &capitalCode)
{
    // SCC decomposition
    auto sccs = findSCC(graph);
    ...
    unordered_map<string, unordered_set<string>> sccAdj;
    ...
    for (const auto &[u, edges] : graph)
    {
        for (const auto &e : edges)
        {
            if (comp[u] != comp[e.to])
            {
                string cu = sccs[comp[u]][0];
                string cv = sccs[comp[e.to]][0];
                if (sccAdj[cu].insert(cv).second)
                {
                    indeg[cv]++;
                }
            }
    ...
    // BFS LAMBDA to explore as much as possible
    auto bfsSCC = [&](const string &start)
    {
        queue<string> q;
        q.push(start);
        ...
        while (!q.empty())
        {
            auto curr = q.front();
            q.pop();
            for (const auto &nxt : sccAdj[curr])
            {
    ...
    // visit everyone that is reachable from capitalSCC
    string capitalSCC = sccs[comp[capitalCode]][0];
    bfsSCC(capitalSCC);

    // collect SCCs that must be connected
    vector<string> result;
    for (const auto &compVec : sccs)
    {
        string repr = compVec[0];
        if (visited.count(repr))
            continue;
        if (indeg[repr] == 0)
        {
